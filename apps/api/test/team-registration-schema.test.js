...
test("019 adds bounded team rosters and participant certificate targets", async () => {
  ...
  const schema = await readFile(new URL("../src/data/schema.sql", import.meta.url), "utf8");
  ...
  assert.match(sql, /DROP CONSTRAINT IF EXISTS registrations_event_project_athlete_key/);
  assert.match(sql, /DROP CONSTRAINT IF EXISTS registrations_event_id_project_id_athlete_key_key/);
  assert.doesNotMatch(schema, /UNIQUE \(event_id, project_id, athlete_key\)/);
});
