...
export function createAccountSecurityRouter({ service, requireUser, asyncRoute }) {
  ...
  router.get("/auth/email/verification/verify", asyncRoute(async (req, res) => {
    const result = await service.inspectVerification({ token: req.query.token });
    if (!result) return res.status(422).json({ error: "链接无效或已过期，请重新申请", code: "INVALID_OR_EXPIRED_TOKEN" });
    res.json({ ok: true, ...result });
  }));
  router.post("/auth/email/verification/confirm", asyncRoute(async (req, res) => {
