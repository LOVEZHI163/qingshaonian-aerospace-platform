...
function statusLevel(usedPercent) {
  if (usedPercent >= UPLOAD_CRITICAL_PERCENT) return "critical";
  if (usedPercent >= UPLOAD_WARNING_PERCENT) return "warning";
  return "normal";
}

export async function readStorageStatus({ uploadRoot, fileSystem = fs }) {
...
export function assertVideoUploadCapacity(status, incomingBytes) {
  ...
  if (status.level === "critical" || projectedPercent >= UPLOAD_CRITICAL_PERCENT) {
