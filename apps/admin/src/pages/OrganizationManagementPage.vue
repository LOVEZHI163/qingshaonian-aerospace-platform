...
function closePreview() { releasePreviewUrl(); preview.value = null; previewError.value = ""; previewLoading.value = false; }
async function openPreview(row) {
  ...
  previewError.value = "";
  try { previewUrl.value = URL.createObjectURL(await apiBlob(`/api/organizations/${row.id}/credential/${document.id}`)); }
  catch (cause) { previewError.value = cause.message || "资质文件预览失败"; }
