...
import DangerConfirmationDialog from "../components/DangerConfirmationDialog.vue";
...
function registrationCount(row) { return (row.eventParticipations || []).reduce((total, participation) => total + Number(participation.registrationCount || 0), 0); }
...
async function loadOrganizations() {
  ...
  try {
    const [organizationPayload, userPayload] = await Promise.all([api("/api/admin/organizations"), api("/api/users")]);
...
async function cleanupCredentials(confirmName) {
  if (saving.value || !selected.value) return;
  saving.value = true; error.value = "";
  try {
    const result = await api(`/api/admin/organizations/${selected.value.id}/credential-cleanup`, {
      method: "POST",
      body: JSON.stringify({ confirmName })
    });
    credentialCleanupOpen.value = false;
    success.value = result.failedFiles?.length
      ? `资质数据已清理，${result.failedFiles.length} 个文件已加入后台重试队列`
      : `已清理 ${result.deletedFiles} 个资质附件`;
    await loadOrganizations();
...
<template>
  <section class="organization-management">
    ...
    <DangerConfirmationDialog
      ...
      title="清理组织资质"
      message="资质附件将被删除，组织会恢复为待审核；重新启用前必须上传新资质并审核通过。用户、组织及成员关系不会删除。"
      :expected-name="selected?.name || ''"
      :busy="saving"
      @cancel="credentialCleanupOpen = false"
