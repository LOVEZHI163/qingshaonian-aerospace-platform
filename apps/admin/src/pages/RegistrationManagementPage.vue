...
import TeamRegistrationFields from "../components/TeamRegistrationFields.vue";
...
const edit = reactive({ organizationId: "", athlete: { name: "", school: "", grade: "", phone: "" }, participants: [], projectId: "", instructor: "" });
...
const editingProject = computed(() => eventProjects.value.find((project) => project.id === edit.projectId) || null);
...
function toggleRoster(registrationId) { expandedId.value = expandedId.value === registrationId ? "" : registrationId; }
function beginEdit(row) { if (archived.value) return; editRow.value = row; Object.assign(edit, { organizationId: row.organizationId || "", athlete: { ...row.athlete }, participants: (row.participants || []).map((person) => ({ ...person })), projectId: row.projectId, instructor: row.instructor || "" }); }
async function saveEdit() {
  ...
  const body = {
    ...
    ...(editRow.value.projectType === "team" ? { participants: edit.participants } : { athlete: edit.athlete })
  };
  try { await api(`/api/admin/events/${encodeURIComponent(props.eventId)}/registrations/${editRow.value.id}`, { method: "PATCH", body: JSON.stringify(body) }); editRow.value = null; success.value = "报名信息已修改"; await loadRows(); } catch (cause) { error.value = cause.message; }
}
...
<template>
  <section class="registration-management"><div class="page-title-row"><div><h2>报名管理</h2><p>审核报名、录入成绩并导出赛事名单。</p></div><button class="dark" data-action="refresh" :disabled="loading" @click="loadRows">刷新</button></div>
    ...
    <div v-if="editRow" class="dialog-backdrop"><form class="panel organization-dialog" @submit.prevent="saveEdit"><h3>编辑报名</h3><p v-if="editRow.projectType === 'team'" class="team-roster-edit-summary">队伍编号 <output data-field="registration-team-code">{{ editRow.teamCode }}</output></p><template v-if="editRow.projectType === 'team'"><TeamRegistrationFields v-model="edit.participants" :min-members="editingProject?.teamMinMembers ?? 1" :max-members="editingProject?.teamMaxMembers ?? 8" :default-school="editRow.organization || ''" /></template><template v-else><div class="two"><label>姓名<input v-model="edit.athlete.name" required></label><label>学校<input v-model="edit.athlete.school" required></label></div><div class="two"><label>年级<input v-model="edit.athlete.grade" required></label><label>手机号<input v-model="edit.athlete.phone" required></label></div></template><label>组织<select v-model="edit.organizationId"><option value="">不关联组织</option><option v-for="organization in organizations" :key="organization.id" :value="organization.id">{{ organization.name }}</option></select></label><label>赛项<select v-model="edit.projectId" data-field="registration-project" disabled><option v-for="project in eventProjects" :key="project.id" :value="project.id">{{ project.name }}</option></select></label><p class="hint">{{ editRow.projectType === "team" ? "赛项和队伍编号在报名创建后不可修改" : "赛项在报名创建后不可修改" }}；如需更换赛项，请取消后重新报名。</p><label>指导老师<input v-model="edit.instructor" data-field="registration-instructor"></label><div class="form-actions"><button class="primary">保存</button><button type="button" @click="editRow = null">取消</button></div></form></div>
