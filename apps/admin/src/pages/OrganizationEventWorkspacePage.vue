...
<template>
  <section class="organization-event-workspace" data-testid="organization-event-workspace">
    ...
    <template v-else>
      ...
      <OrganizationAthleteRegistrationForm v-if="activeTab === 'registration' && !archived" :event-id="props.eventId" :projects="workspace.projects || []" :grades="workspace.grades || []" :default-school="workspace.organization?.name || ''" @registered="registered" @error="emit('error', $event)" />
      ...
      <section v-else-if="activeTab === 'records'" class="panel workspace-table"><div class="panel-title"><h3>报名记录</h3><button type="button" class="mini" data-action="export-organization-registrations" @click="exportRegistrations">导出名单</button></div><OrganizationAthleteRegistrationForm v-if="editingRegistration && !archived" :event-id="props.eventId" :projects="workspace.projects || []" :grades="workspace.grades || []" :default-school="workspace.organization?.name || ''" :registration="editingRegistration" @registered="savedRegistration" @error="emit('error', $event)" />
