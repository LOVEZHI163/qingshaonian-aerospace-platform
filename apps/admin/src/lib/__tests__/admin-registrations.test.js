...
describe("loadAdminRegistrations", () => {
  it("loads every page, preserving filters and deduplicating rows", async () => {
    ...
    const request = vi.fn(async (path) => {
      if (path === "/api/admin/events/E1/registrations?status=pending&pageSize=100") {
        return { rows: firstPage, total: 101, page: 1, pageSize: 100 };
      }
      if (path === "/api/admin/events/E1/registrations?status=pending&page=2&pageSize=100") {
        return { rows: [{ id: "R101" }], total: 101, page: 2, pageSize: 100 };
      }
      throw new Error(`unexpected path ${path}`);
    });

    const rows = await loadAdminRegistrations({ ...eventFilters, status: "pending" }, request);

    expect(rows).toHaveLength(101);
    expect(request).toHaveBeenNthCalledWith(1, "/api/admin/events/E1/registrations?status=pending&pageSize=100");
    expect(request).toHaveBeenNthCalledWith(2, "/api/admin/events/E1/registrations?status=pending&page=2&pageSize=100");
  });

  it("fails closed when the initial response omits total", async () => {
    const request = vi.fn(async () => ({ rows: [{ id: "R1" }], page: 1, pageSize: 100 }));

    await expect(loadAdminRegistrations(eventFilters, request)).rejects.toBeInstanceOf(AdminRegistrationPaginationError);
  ...
  it("fails closed when a later page is empty after the reported total changes", async () => {
    ...
    await expect(loadAdminRegistrations(eventFilters, request)).rejects.toThrow("报名数据在加载期间发生变化，请刷新重试");
  ...
  it("rejects maliciously large initial page counts without requesting more pages", async () => {
    ...
    await expect(loadAdminRegistrations(eventFilters, request)).rejects.toThrow("报名数据过多，请缩小赛事或筛选范围后重试");
    expect(request).toHaveBeenCalledTimes(1);
  });

  it("returns complete zero and single-page results", async () => {
    await expect(loadAdminRegistrations(eventFilters, vi.fn(async () => ({ rows: [], total: 0, page: 1, pageSize: 100 })))).resolves.toEqual([]);
    await expect(loadAdminRegistrations(eventFilters, vi.fn(async () => ({ rows: [{ id: "R1" }], total: 1, page: 1, pageSize: 100 })))).resolves.toEqual([{ id: "R1" }]);
  });
});
