...
function keydown(event) {
  ...
  const focusable = [...(dialog.value?.querySelectorAll("button:not([disabled]),a[href],input:not([disabled]),[tabindex]:not([tabindex='-1'])") || [])];
...
<template>
  <div v-if="open" class="dialog-backdrop" @click.self="close">
    <section ref="dialog" class="panel content-preview-dialog" role="dialog" aria-modal="true" aria-labelledby="content-preview-title">
